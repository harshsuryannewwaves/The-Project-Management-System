...
export default function TicketManagementSection() {
  ...
  const fetchTickets = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/tickets', axiosConfig);
      setTickets(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTicket) {
        await axios.put(`http://localhost:5000/api/tickets/${editingTicket._id}`, fd, {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        await axios.post('http://localhost:5000/api/tickets/create', fd, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'multipart/form-data'
          }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/tickets/${id}`, axiosConfig);
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {editingTicket?.image && (
                ...
                <div className="mb-2"> <img src={`http://localhost:5000${editingTicket.image}`} alt="ticket" className="w-32 h-auto rounded" /> </div>)}
